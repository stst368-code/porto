...
# Magazine
check("capacity-driven wheel positions built", "index < MAIN_CAPACITY" in js)
...
check("wheel cards contain artwork only", "gbr-slot-copy" not in html and "gbr-slot-title" not in html and "gbr-slot-state" not in html and "gbr-slot-copy" not in js)
...
check("cassette clicks are not swallowed by wheel drag capture", 'event.target.closest(".gbr-slot-card")' in js and 'return;' in js[js.index('event.target.closest(".gbr-slot-card")'):js.index('event.target.closest(".gbr-slot-card")')+120])
...
check("cassette wheel has a full lamp ring", ".gbr-wheel-stage::before" in css and "--ring-opacity" in css and "--ring-pulse" in css)
