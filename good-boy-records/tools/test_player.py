...
# Catalogue conveyor
check("wall has a bounded visible window", "const CONVEYOR_VISIBLE = 20" in js and "phase >= CONVEYOR_VISIBLE" in js)
...
check("wall cards contain artwork only", "gbr-track-tag" not in js[js.index("function buildWheel"):js.index("function normaliseWheelIndex")])
...
check("desktop wall layout exists", "COVER_WALL_COLUMNS = 5" in js and "COVER_WALL_ROWS = 4" in js and ".gbr-wheel-stage::before" in css)
check("wall keeps idle animation disabled", "function animateConveyor(now)" in js and "state.conveyorLastFrame = now;" in js)
check("wall retains interaction pause state", "markConveyorInteraction" in js and "CONVEYOR_RESUME_DELAY" in js)
...
check("loaded track syncs into wall focus", "function syncWheelToTrack" in js and "CONVEYOR_FOCUS_SLOT" in js and "syncWheelToTrack(track);" in js[js.index("function selectTrack"):js.index("async function playAudio")])
...
check("wheel center runtime helper retained for non-desktop modes", 'id="gbr-wheel-center-artwork"' in js and 'function renderWheelCenter' in js)
check("wheel center title helper retained", 'id="gbr-wheel-center-title"' in js and 'gbr-wheel-center-copy' in js)
check("loaded track updates browse identity", 'renderWheelCenter(track);' in js)
...
check("desktop wall replaces the old lamp ring", ".gbr-slot-card[data-at-gate=\"true\"]" in css and ".gbr-wheel-stage::before" in css)
...
check("short desktop still enlarges lyrics", "grid-template-rows:minmax(0,calc(100dvh - 136px)) 106px" in css)
