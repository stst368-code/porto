...
check("one-off bank is unlimited", 'if len(one_off_songs) > MAX_SONGS' not in builder and "ONE_OFF_PAGE_SIZE" not in js and "oneOffPageCount" not in js)
...
check("one-off uses dedicated cassette wall", 'id="gbr11-oneoff-wall"' in template and "function renderOneOffWall" in js and ".gbr11-oneoff-wall" in css)
check("one-off wall renders every side as its own cassette", "oneOffWallItems" in js and "keys.forEach((side) => items.push" in js and 'card.dataset.side = side' in js)
check("one-off wall has no ten-item pager", "setOneOffPage" not in js and "gbr11-oneoff-pager" not in js and "MAGAZINE ${" not in js)
check("one-off wall replaces rotary surfaces only in one-off mode", 'html[data-browse-genre="one-off"]:not([data-easter="true"]) .gbr11-wheel-stage' in css and '.gbr11-mobile-rail { display:none !important; }' in css)
check("one-off wall keeps deterministic loose cassette offsets", "--wall-rot" in js and "--wall-x" in js and "--wall-y" in js and "transform:translate(var(--wall-x),var(--wall-y)) rotate(var(--wall-rot))" in css)
check("one-off wall mobile double tap still plays", 'function handleWallCassette(track, key)' in js and 'matchMedia("(max-width: 900px)")' in js and 'playAudio();' in js[js.index('function handleWallCassette'):js.index('function renderOneOffWall')])
check("one-off playback sequence includes separate sides", 'function tracksForGenre(genre)' in js and 'sideKeys(sides).forEach((side) => out.push(sides[side]))' in js)
